...
blob_relative_path = "www2.census.gov/programs-surveys/decennial/"
...
def filtered_data(clean_data):
    ...
    for ind , row in enumerate(clean_data):
        ...
        for value in data :
            ...
            if ((value.count(None) > 1 or (none_count <= 1 and value.count(None) == 1)  or value[0] is None) or all(not x.isdigit() for x in value if x!=None)) and (value[-2]== None or not value[-2].isdigit()):
...
if len(bad_records)>= 1:
    ...
    print(bad_path,'uploaded sucessfully')
