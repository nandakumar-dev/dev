...
def get_vertical_horizontal_plotting_lines(pdf_plumber_object, page_number, region, adjustments=None,
    ...
    return flatten_horizontal_lines + flatten_vertical_lines
...
def draw_lines_on_pdf(pdf_blob, demarcation_settings):
    ...
    with pdfplumber.open(BytesIO(pdf_blob)) as pdf:
        ...
        for page_number, page_info in demarcation_settings.items():
            page_lines_orientation = {'rotation': None}
            lines_on_a_page = []
            for page_info_key, page_info_value in page_info.items():
                if page_info_key == 'rotation':
                    page_lines_orientation['rotation'] = page_info_value
                    continue
                ...
                lines = get_vertical_horizontal_plotting_lines(
                    pdf, int(page_number), region, adjustments, vertical_threshold=vertical_threshold
                )
                lines_on_a_page.extend(lines)
            page_lines_orientation['lines'] = lines_on_a_page
            page_demarcation_info[int(page_number) - 1] = page_lines_orientation
...
def extract_table_from_region(pdf_blob, page_number, region):
    """
    ...
    with pdfplumber.open(BytesIO(pdf_blob)) as pdf:
        ...
        if region is not None:
            cropped_page = page.within_bbox((region['x0'], region['top'], region['x1'], region['bottom']))
